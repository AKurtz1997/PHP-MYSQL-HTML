...
if ($_SESSION['user_admin']) {
    if ($_GET[id]) { // user is admin and I have an ID to delete from database
        $sql = "delete from $reservationsTable where id = ".$_GET[id];
        if ($stmt = mysqli_prepare($conn, $sql)) {
            mysqli_stmt_execute($stmt);
            mysqli_stmt_store_result($stmt);
            echo mysqli_stmt_affected_rows($stmt)." rows were deleted from Reservation Table table\n<br>";
            mysqli_stmt_close($stmt);
        }
   
    
    $AvailableCheck = "SELECT available FROM Dorms Where id = ".$_GET[dormid]; 
    ...
    $availableSQL = "Update Dorms SET available = ('$available' + 1) WHERE id =  ".$_GET[dormid];
    
    $userDormUpdateSQL = "Update users SET userDorm = null WHERE cwid =" . $_GET[cwid];
    
    if($conn -> query($availableSQL) && $conn -> query($userDormUpdateSQL) == TRUE){
        echo "updating Dorm avaliable";   
        echo "<BR>";
        echo "updating user dorm preference";
        echo "<BR>";
